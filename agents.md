...
**Latest Update**: WP POS v1.9.137 - Made Refund Details Modal Scrollable - implemented proper viewport height constraints with max-h-[90vh] and flex layout, modal now uses flex-column structure with fixed header/footer (flex-shrink-0) and scrollable content area (flex-1 overflow-y-auto), prevents modal from extending beyond viewport on smaller screens or with large amounts of refund data, maintains modern card-based design from v1.9.136
