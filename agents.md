...
**Latest Update**: WP POS v1.9.71 - Removed Customer Attachment Feature - completely removed customer attachment functionality due to persistent WooCommerce conflicts, simplified checkout to always use cashier/admin as order customer in [`api/checkout.php:44-165`](api/checkout.php:44-165), all POS orders now show cashier as customer in WooCommerce admin which provides clear audit trail of who processed each transaction, uses store address for all orders from settings, removed complex customer lookup and address mapping logic, maintains `_jpos_created_by` meta for cashier tracking, eliminates all customer assignment conflicts since order customer is set once at creation and never needs to change, simpler and more reliable than attempting to override WooCommerce's aggressive customer detection
