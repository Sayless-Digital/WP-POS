...
**Latest Update**: WP POS v1.9.142 - Fixed Print Reports to Use New Window Approach - replaced @media print CSS with new window printing strategy matching receipt functionality, implemented [`printReport()`](assets/js/modules/financial/reports.js:568-603) method in ReportsManager that opens clean window with styled content and calls window.print(), implemented identical [`printReport()`](assets/js/modules/financial/refund-reports.js:463-498) method in RefundReportsManager, updated print button handler in [`main.js:506-515`](assets/js/main.js:506-515) to call appropriate manager printReport() method based on modal title, completely removed @media print CSS block from [`index.php:247-287`](index.php:247-287) as no longer needed with new window approach, eliminates printing entire page including login screen and menus that occurred with window.print() on modal, provides clean professional print output containing only report content across multiple pages, resolves user-reported issue of excessive content in print output
