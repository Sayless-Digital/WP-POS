...
- **v1.9.193** (2025-10-27): Auto-Refresh Timer Initialization Fix Attempt - attempted to fix countdown not showing by ensuring proper initialization sequence, but timer still not displaying due to missing explicit init() call in auth flow

- **v1.9.192** (2025-10-27): Implemented Automatic App Refresh System - added comprehensive auto-refresh functionality with user-configurable intervals allowing users to set automatic page reloads from 1-60 minutes or define custom intervals, backend implementation in [`api/settings.php:23-24,84-88`](api/settings.php:23-24) adds `auto_refresh_enabled` (boolean) and `auto_refresh_interval` (integer in minutes) to settings with defaults of false and 5 minutes, frontend UI in [`index.php:967-1015`](index.php:967-1015) provides General settings section with enable checkbox and interval configuration via predefined quick select buttons (1/5/10/30 minutes) plus custom minutes input field with range validation (1-60 minutes acceptable), JavaScript implementation in [`assets/js/modules/auto-refresh.js:1-196`](assets/js/modules/auto-refresh.js:1-196) includes `AutoRefreshManager` class managing countdown timer with methods for start/stop/reset/updateSettings, visual countdown indicator at bottom-left of screen displays "Auto-refresh in: Xm Ys" with amber background when under 1 minute and slate background otherwise, countdown updates every second via `setInterval`, automatically triggers `window.location.reload()` when timer reaches zero, settings integration in [`assets/js/modules/admin/settings.js:141-310`](assets/js/modules/admin/settings.js:141-310) handles save/load of preferences, provides complete solution for keeping POS data fresh in environments where stock levels or product information change frequently, updated cache-busting versions to v1.9.192 for [`settings.js`](index.php:50), [`main.js`](index.php:55), [`auto-refresh.js`](index.php:53), system version updated to v1.9.192 in [`index.php:20`](index.php:20)

- **v1.9.191** (2025-10-26): Fixed Receipt Item Negative Totals Display - resolved bug where return items on receipts showed "$0.00" instead of negative amounts like "-$195.00", modified [`showReceipt()`](assets/js/modules/orders/receipts.js:37-49) to properly format item totals by checking if `itemTotal < 0` and formatting as `-$${Math.abs(itemTotal).toFixed(2)}` for negative values or `$${itemTotal.toFixed(2)}` for positive values, receipts now accurately display negative item totals for returns making it clear which items were returned and their refund values, provides transparent financial breakdown on printed receipts matching customer expectations, updated cache-busting version from v1.9.190 to v1.9.191 in [`index.php:20,42`](index.php:20), system version updated to v1.9.191

- **v1.9.190** (2025-10-26): Fixed Exchange Payment Calculation - resolved critical bug in [`openSplitPaymentModal()`](assets/js/modules/cart/checkout.js:89-111) where second payment split amount was calculated incorrectly for exchanges, root cause was using `cartTotal - refundCredit` when cartTotal already had returns subtracted causing double-subtraction (e.g., with $195 return and $1200 new items, cartTotal was $1005 but code subtracted $195 again resulting in incorrect $810 payment), corrected to calculate new items total separately and use `newItemsTotal - refundCredit` for accurate remaining payment amount, now properly calculates payment splits: first split pre-fills with "Return/Refund Credit" method showing full return credit amount ($195), second split pre-fills with "Cash" method showing correct remaining balance after credit applied ($1005 for $1200 new items - $195 credit), provides accurate payment calculations matching total display and customer expectations, updated cache-busting version from v1.9.189 to v1.9.190 in [`index.php:20,37`](index.php:20), system version updated to v1.9.190

- **v1.9.189** (2025-10-26): Improved Refund Receipt Labels - replaced confusing "Refund Due" and "Payment Due" labels with clearer action-completed terminology at [`receipts.js:374`](assets/js/modules/orders/receipts.js:374), refund receipts now display "AMOUNT REFUNDED:" (green) for simple refunds where customer received money back, "NET REFUND:" (green) for exchanges where customer still gets money back after exchange, or "ADDITIONAL PAYMENT:" (red) for exchanges where customer owes additional money for new items, eliminates confusion where "Refund Due" sounded like something still pending rather than already processed, provides clearer understanding that refund transaction is complete and money has already been handled, updated cache-busting version from v1.9.188 to v1.9.189 in [`index.php:20,42`](index.php:20), system version updated to v1.9.189

- **v1.9.188** (2025-10-26): Redesigned Refund Receipt Layout - completely redesigned refund/exchange receipt display at [`showRefundReceipt()`](assets/js/modules/orders/receipts.js:262-354) to match clean simple style of regular receipts, eliminated complex inline styles (`style="..."` attributes) and replaced with consistent Tailwind CSS grid-based layout using `grid-cols-12` pattern matching regular receipt structure at [`showReceipt()`](assets/js/modules/orders/receipts.js:33-230), implemented proper header with logo and store name, used same item display format as regular receipt with grid columns for #/Item/Qty/Amount, applied consistent border styling (`border-dashed border-gray-400`), added proper section headings ("RETURNED ITEMS", "NEW ITEMS" for exchanges), color-coded amounts (red for refunds using `text-red-600`, green for credits using `text-green-600`), integrated store settings (logo, name, footer) consistently, maintained clear information hierarchy with proper spacing, improved readability and user understanding of refund/exchange information making it much easier to comprehend transaction details at a glance, updated cache-busting versions from v1.9.187 to v1.9.188 in [`index.php:20,42`](index.php:20), system version updated to v1.9.188
