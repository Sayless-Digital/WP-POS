...
**Latest Update**: WP POS v1.9.191 - Fixed Receipt Item Negative Totals Display - corrected receipt item total display in [`showReceipt()`](assets/js/modules/orders/receipts.js:37-49) to properly format negative amounts for return items, return items now show as "-$195.00" instead of "$0.00", uses proper negative formatting with Math.abs() and prepends minus sign, provides accurate item-level financial information on receipts for returns and exchanges
