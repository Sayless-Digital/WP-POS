...
**Latest Update**: WP POS v1.8.56 - Fixed customer display not clearing after holding cart - modified [`clearCart()`](assets/js/main.js:1372) to properly clear customer data when fullReset parameter is true, customer display now correctly removed from UI when cart is held, resolves visual display bug where customer box remained visible even though customer data was cleared from state
